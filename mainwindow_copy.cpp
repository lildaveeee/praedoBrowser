...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), sidebarVisible(true), sidebarShowLock(false)
{
    qputenv("QTWEBENGINE_CHROMIUM_FLAGS", "--disable-component-update --enable-widevine-cdm --disable-gpu --disable-software-rasterizer");
    QWebEngineProfile *profile = QWebEngineProfile::defaultProfile();
    QString storagePath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/PraedoBrowserProfile";
    ...
    // YouTube shortcut
    QString youtubeKey = settings.value("youtubeShortcut", "Ctrl+Y").toString();
    QShortcut *youtubeShortcut = new QShortcut(QKeySequence(youtubeKey), this);
    connect(youtubeShortcut, &QShortcut::activated, this, [this]() {
        QUrl ytUrl("https://www.youtube.com");
        QWebEngineView *view = qobject_cast<QWebEngineView*>(webStack->currentWidget());
        if (view) {
            view->setUrl(ytUrl);
        } else {
            QWebEngineView *newView = createWebView(ytUrl);
            int index = webStack->addWidget(newView);
            QListWidgetItem *item = new QListWidgetItem("YouTube");
...
void MainWindow::applyModernBlueTheme() {
    QSettings settings("PraedoBrowser", "PraedoBrowser");
    QString backgroundColor        = settings.value("backgroundColor", "#19140F").toString();
    QString topBarColor        = settings.value("topBarColor", "#2A2118").toString();
    QString accent1Color       = settings.value("accent1Color", "#D4B398").toString();
    QString accent2Color       = settings.value("accent2Color", "#C8BBA9").toString();
    ...
    QString alternateBaseColor = settings.value("alternateBaseColor", "#19140F").toString();
    QString highlightColor = settings.value("highlightColor", "#19140F").toString();
    QString buttonColor       = settings.value("buttonColor", "#1E1812").toString();
    ...
    QString styleSheet = QString(
                             ...
                             "QListWidget { background: white;  border-top: 1px solid %3;  border-right: 2px solid %3; border-bottom: none; border-left: none; color: %6; border-top-right-radius: 7px; padding: 10px; }"
                             "QListWidget::item { padding: 8px; }"
                             "QListWidget::item:selected { background-color: transparent; padding-left: 12px; color: %6; }"
                             "QPushButton { background-color: %9 ; border-top: 1px solid %3;  border-right: 1px solid %3; border-bottom: none; border-left: none; border-bottom-right-radius: 3px; color: %6; }"
