...
static QMap<QString, QString> createBangMap() {
    ...
    searchMap.insert("media",    "https://hydrahd.sh/index.php?menu=search&query={search}");
    ...
    if (input.startsWith("!")) {
        ...
        if (parts.size() >= 2) {
            ...
            if (bangMap.contains(command)) {
                ...
                QString urlString = urlTemplate;
                urlString.replace("{search}", QUrl::toPercentEncoding(searchQuery), Qt::CaseSensitive);
                QUrl url(urlString);
                QWebEngineView *view = qobject_cast<QWebEngineView*>(webStack->currentWidget());
                if (view)
                    view->setUrl(url);
...
void MainWindow::goHome() {
    QSettings settings("MyCompany", "PraedoBrowser");
    // Retrieve the default search engine URL from settings.
    QString defaultUrlString = settings.value("defaultSearchEngine",
                                              "https://www.duckduckgo.com").toString();
    QUrl defaultUrl(defaultUrlString);
